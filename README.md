...
リモートデスクトップで DirectInput のホイール値が非ゼロのまま残る環境では、`SetRemoteDesktopMode(true)` を使う。  
このモードでは同じ非ゼロのホイール値が連続して返った場合、2フレーム目以降を 0 として扱い、ホイールを動かし続けているように見える状態を抑制する。
...
`GamePad::ToggleTestDialog(parentWindow)` を呼ぶと、ゲームパッドが接続されていなくてもボタン入力を送れるテストウィンドウを表示できる。

通常モードでは画面上のボタンを押している間だけ入力される。`トグルモード`を選ぶと、クリックするたびに押下状態を切り替えられる。

`キーボードモード`では、WASDを左スティック、TFGHを右スティック、方向キーを十字キーとして扱う。I/J/K/LはY/X/A/B、Q/EはL1/L2、U/OはR1/R2、R/YはSTART/SELECT(BACK)に対応する。

ウィンドウを閉じたとき、モードを切り替えたとき、および`すべて解除`を押したときは、すべての疑似入力が解除される。
